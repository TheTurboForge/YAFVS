...
class YAFVSCtlTests(unittest.TestCase):
    def test_component_registry_has_expected_components(self):
        names = [component.name for component in yafvsctl.COMPONENTS]
    ...
    def _native_scan_new_system_result(
        ...
        *,
        ...
        def fake_direct(_root, path, **kwargs):
            ...
            if method == "GET" and path == f"/api/v1/port-lists/{yafvsctl.IANA_TCP_UDP_PORT_LIST_ID}":
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": yafvsctl.IANA_TCP_UDP_PORT_LIST_ID, "name": "IANA"}) + "\n200", "")
            if method == "GET" and path == f"/api/v1/scan-configs/{yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID}":
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID, "name": "Full and fast"}) + "\n200", "")
            if method == "GET" and path == f"/api/v1/scanners/{yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID}":
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID, "name": "OpenVAS", "scanner_type": 2}) + "\n200", "")
            ...
            if method == "POST" and path == "/api/v1/tasks":
                ...
                self.assertEqual(body["config_id"], yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID)
                self.assertEqual(body["scanner_id"], yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                ...
                result = yafvsctl.command_native_scan_new_system(Path(tmp), host="192.0.2.10", allow_scan_control=True)
        return result, calls, ids

    def test_native_scan_new_system_rejects_invalid_arguments_and_preflight_before_writes(self):
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                invalid = yafvsctl.command_native_scan_new_system(Path(tmp), host="192.0.2.0/24", allow_scan_control=True)
                curl.assert_not_called()
        ...
        def fake_direct(_root, path, **kwargs):
            ...
            if path.endswith("/port-lists/" + yafvsctl.IANA_TCP_UDP_PORT_LIST_ID):
                payload = {"id": yafvsctl.IANA_TCP_UDP_PORT_LIST_ID, "name": "IANA"}
            elif path.endswith("/scan-configs/" + yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID):
                payload = {"id": yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID, "name": "Full and fast"}
            else:
                payload = {"id": yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID, "name": "Not a task scanner", "scanner_type": 3}
    ...
    def test_native_scan_new_system_dry_run_plans_without_writes(self):
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_scan_new_system(Path(tmp), host="2001:db8::10", dry_run=True)
                curl.assert_not_called()
        self.assertEqual(result["status"], "pass")
    ...
    def test_native_scan_new_system_preflights_then_creates_and_starts_in_order(self):
        ...
        self.assertEqual(calls, [("GET", f"/api/v1/port-lists/{yafvsctl.IANA_TCP_UDP_PORT_LIST_ID}"), ("GET", f"/api/v1/scan-configs/{yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID}"), ("GET", f"/api/v1/scanners/{yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID}"), ("POST", "/api/v1/targets"), ("POST", "/api/v1/tasks"), ("POST", f"/api/v1/tasks/{ids['task']}/start")])
    ...
    def test_native_scan_new_system_parser_is_registered(self):
        args = yafvsctl.build_parser().parse_args(["native-scan-new-system", "--host", "192.0.2.10", "--dry-run"])
    ...
    def test_native_scan_new_system_is_not_a_remaining_gvm_tools_replacement_candidate(self):
        ...
        self.assertNotIn("scan-new-system.gmp.py", yafvsctl.NATIVE_TOOLING_GVM_TOOLS_PATH_BLOCKERS)
    ...
    def test_native_nvt_diagnostic_scan_defaults_to_empty_and_dry_runs_without_runtime_access(self):
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env") as runtime_env, \
                ...
                result = yafvsctl.command_native_nvt_diagnostic_scan(
                    Path(tmp),
                    host="192.0.2.10",
        ...
        self.assertEqual(result["status"], "pass")
        self.assertEqual(result["details"]["status"], "dry_run")
        self.assertEqual(result["details"]["source_scan_config_id"], yafvsctl.EMPTY_SCAN_CONFIG_ID)
        self.assertEqual(
            result["details"]["planned_selection"]["prerequisite_nvt_ids"],
            list(yafvsctl.NATIVE_NVT_DIAGNOSTIC_PREREQUISITE_IDS),
        )
    ...
    def test_native_nvt_diagnostic_scan_preflights_clones_selects_creates_and_starts(self):
        ...
        def fake_direct(_root, path, **kwargs):
            ...
            if method == "GET" and path == f"/api/v1/scan-configs/{yafvsctl.EMPTY_SCAN_CONFIG_ID}":
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": yafvsctl.EMPTY_SCAN_CONFIG_ID, "name": "Empty", "predefined": True, "writable": False, "in_use": False}) + "\n200", "")
            if method == "GET" and path == f"/api/v1/port-lists/{yafvsctl.IANA_TCP_UDP_PORT_LIST_ID}":
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": yafvsctl.IANA_TCP_UDP_PORT_LIST_ID, "name": "IANA"}) + "\n200", "")
            if method == "GET" and path == f"/api/v1/scanners/{yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID}":
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID, "name": "OpenVAS", "scanner_type": 2}) + "\n200", "")
        ...
        self.assertEqual(
            ...
            [
                ...
                ("GET", f"/api/v1/scan-configs/{yafvsctl.EMPTY_SCAN_CONFIG_ID}"),
                ("GET", f"/api/v1/port-lists/{yafvsctl.IANA_TCP_UDP_PORT_LIST_ID}"),
                ("GET", f"/api/v1/scanners/{yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID}"),
            ],
        )
    ...
    def test_native_nvt_diagnostic_scan_cleans_config_and_target_after_proven_task_failure(self):
        ...
        def fake_direct(_root, path, **kwargs):
            ...
            if method == "GET" and "/nvts/" in path:
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": "1.3.6.1.4.1.25623.1.0.106223", "name": "Diagnostic NVT"}) + "\n200", "")
            if method == "GET" and path.endswith(f"/scan-configs/{yafvsctl.EMPTY_SCAN_CONFIG_ID}"):
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"id": yafvsctl.EMPTY_SCAN_CONFIG_ID, "name": "Empty", "predefined": True, "writable": False, "in_use": False}) + "\n200", "")
    ...
    def test_native_nvt_diagnostic_scan_parser_and_removal_inventory(self):
        args = yafvsctl.build_parser().parse_args(
            [
        ...
        self.assertEqual(args.source_scan_config_id, yafvsctl.EMPTY_SCAN_CONFIG_ID)
        candidates = set().union(*yafvsctl.NATIVE_TOOLING_GVM_TOOLS_REMOVAL_BUCKETS.values())
        self.assertNotIn("nvt-scan.gmp.py", candidates)
        self.assertNotIn("start-nvt-scan.gmp.py", candidates)
        self.assertNotIn("nvt-scan.gmp.py", yafvsctl.NATIVE_TOOLING_GVM_TOOLS_PATH_BLOCKERS)
        self.assertNotIn("start-nvt-scan.gmp.py", yafvsctl.NATIVE_TOOLING_GVM_TOOLS_PATH_BLOCKERS)
    ...
    def _native_scan_with_delivery_result(self, *, host_mode=False, alert=None, target_response=None, task_response=None, start_response=None, follow_up_response=None, reconcile=True, pre_start_alert=None):
        ...
        def fake_direct(_root, path, **kwargs):
            ...
            if method == "GET" and path.startswith("/api/v1/targets?filter="):
                items = [] if not reconcile else [{"id": ids["target"], "name": created_names["target"], "owner_id": ids["operator"], "hosts": ["192.0.2.10"], "port_list": {"id": yafvsctl.IANA_TCP_UDP_PORT_LIST_ID, "name": "IANA"}}]
            ...
            if method == "GET" and path.startswith("/api/v1/tasks?filter="):
                items = [] if not reconcile else [{"id": ids["task"], "name": created_names["task"], "owner_id": ids["operator"], "target": {"id": ids["target"]}, "config": {"id": yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID}, "scanner": {"id": yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID}}]
                return subprocess.CompletedProcess(["curl"], 0, json.dumps({"items": items, "page": {"total": len(items)}}) + "\n200", "")
            if method == "POST" and path == f"/api/v1/tasks/{ids['task']}/start":
                return start_response or subprocess.CompletedProcess(["curl"], 0, json.dumps({"task_id": ids["task"], "report_id": ids["report"], "status": "requested"}) + "\n202", "")
            if method == "GET" and path == f"/api/v1/tasks/{ids['task']}":
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={yafvsctl.YAFVS_API_OPERATOR_UUID_ENV: ids["operator"]}), \
                ...
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="a" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                result = yafvsctl.command_native_scan_with_delivery(Path(tmp), alert_id=ids["alert"], allow_scan_control=True, **kwargs)
    ...
    def test_native_scan_with_delivery_defaults_to_dry_run_without_runtime_access(self):
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env") as runtime_env, \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_scan_with_delivery(Path(tmp), host="192.0.2.10", alert_id="44444444-4444-4444-8444-444444444444")
        runtime_env.assert_not_called()
        curl.assert_not_called()
        self.assertEqual(result["details"]["status"], "dry_run")
        ...
        self.assertEqual(result["details"]["port_list_id"], yafvsctl.IANA_TCP_UDP_PORT_LIST_ID)
        self.assertEqual(result["details"]["scan_config_id"], yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID)
        self.assertEqual(result["details"]["scanner_id"], yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID)
    ...
    def test_native_scan_with_delivery_existing_target_preflights_then_starts_without_target_post(self):
        ...
        self.assertEqual(result["details"]["status"], "requested")
        self.assertEqual(calls, [("GET", f"/api/v1/targets/{ids['target']}"), ("GET", f"/api/v1/scan-configs/{yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID}"), ("GET", f"/api/v1/scanners/{yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID}"), ("GET", f"/api/v1/alerts/{ids['alert']}"), ("POST", "/api/v1/tasks"), ("GET", f"/api/v1/alerts/{ids['alert']}"), ("POST", f"/api/v1/tasks/{ids['task']}/start")])
        self.assertEqual(payloads, [{"name": payloads[0]["name"], "target_id": ids["target"], "config_id": yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID, "scanner_id": yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID, "alert_ids": [ids["alert"]]}])
    ...
    def test_native_scan_with_delivery_host_creates_target_after_preflight(self):
        ...
        self.assertEqual(result["details"]["target_id"], ids["target"])
        self.assertEqual(calls[:5], [("GET", f"/api/v1/port-lists/{yafvsctl.IANA_TCP_UDP_PORT_LIST_ID}"), ("GET", f"/api/v1/scan-configs/{yafvsctl.FULL_AND_FAST_SCAN_CONFIG_ID}"), ("GET", f"/api/v1/scanners/{yafvsctl.NATIVE_SCAN_NEW_SYSTEM_DEFAULT_SCANNER_ID}"), ("GET", f"/api/v1/alerts/{ids['alert']}"), ("POST", "/api/v1/targets")])
    ...
    def test_native_scan_with_delivery_parser_and_recipe_are_registered(self):
        args = yafvsctl.build_parser().parse_args(["native-scan-with-delivery", "--target-id", "11111111-1111-4111-8111-111111111111", "--alert-id", "44444444-4444-4444-8444-444444444444"])
        ...
        self.assertIn('tools/yafvsctl native-scan-with-delivery "$@"', justfile)
        candidates = set().union(*yafvsctl.NATIVE_TOOLING_GVM_TOOLS_REMOVAL_BUCKETS.values())
    ...
    def test_native_scan_with_delivery_status_only_keeps_delivery_reference(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            result = yafvsctl.command_native_scan_with_delivery(
                Path(tmp),
                host="192.0.2.10",
                alert_id=alert_id,
                status_only=True,
    ...
    def test_native_export_report_pdf_parser_success_and_token_redaction(self):
        report_id = "11111111-1111-4111-8111-111111111111"
        token = "t" * 64
        args = yafvsctl.build_parser().parse_args(["native-export-report-pdf", "--report-id", report_id])
        self.assertEqual(args.command, "native-export-report-pdf")
        self.assertEqual(args.max_bytes, yafvsctl.NATIVE_REPORT_PDF_DEFAULT_MAX_BYTES)
        self.assertIsNone(args.output)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                unittest.mock.patch.object(yafvsctl, "native_api_direct_config_shape_finding", return_value=yafvsctl.finding("pass", "direct-config", "ok")), \
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value=token), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_binary_download", side_effect=fake_download):
                result = yafvsctl.command_native_export_report_pdf(root, report_id=report_id, output=output, overwrite=True, status_only=True)
            self.assertEqual(result["status"], "pass")
    ...
    def test_native_export_report_pdf_never_overwrites_raced_destination_without_permission(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                unittest.mock.patch.object(yafvsctl, "native_api_direct_config_shape_finding", return_value=yafvsctl.finding("pass", "direct-config", "ok")), \
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="w" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_binary_download", side_effect=raced_download):
                result = yafvsctl.command_native_export_report_pdf(root, report_id=report_id, output=output)
            self.assertEqual(result["status"], "fail")
    ...
    def test_native_export_report_pdf_preserves_old_output_on_cap_error_and_replace_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with common_patches[0], common_patches[1], common_patches[2], \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_binary_download", side_effect=fake_download):
                capped = yafvsctl.command_native_export_report_pdf(root, report_id=report_id, output=output, max_bytes=len(pdf), overwrite=True)
            self.assertEqual(capped["status"], "fail")
            ...
            with common_patches[0], common_patches[1], common_patches[2], \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_binary_download", side_effect=valid_download), \
                unittest.mock.patch.object(yafvsctl.os, "replace", side_effect=OSError("replace denied")):
                replacement = yafvsctl.command_native_export_report_pdf(root, report_id=report_id, output=output, overwrite=True)
    ...
    def test_native_export_report_pdf_rejects_json_error_without_runtime_text_decoding(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                unittest.mock.patch.object(yafvsctl, "native_api_direct_config_shape_finding", return_value=yafvsctl.finding("pass", "direct-config", "ok")), \
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="v" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_binary_download", side_effect=fake_download):
                result = yafvsctl.command_native_export_report_pdf(root, report_id=report_id, output=output, overwrite=True, status_only=True)
            self.assertEqual(result["status"], "fail")
            self.assertEqual(output.read_bytes(), b"old output")
    ...
    def test_native_export_report_bundle_parser_and_pre_runtime_guards(self):
        ...
        self.assertEqual(args.max_items, yafvsctl.NATIVE_REPORT_BUNDLE_DEFAULT_MAX_ITEMS)
        self.assertEqual(args.max_bytes, yafvsctl.NATIVE_REPORT_BUNDLE_DEFAULT_MAX_BYTES)
        self.assertIsNone(args.output)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                invalid = yafvsctl.command_native_export_report_bundle(root, report_id="not-a-uuid")
                exists = yafvsctl.command_native_export_report_bundle(root, report_id=report_id, output=existing)
                capped = yafvsctl.command_native_export_report_bundle(root, report_id=report_id, max_items=0)
                curl.assert_not_called()
    ...
    def test_native_export_report_bundle_is_complete_atomic_and_redacted(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                ...
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="s" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                result = yafvsctl.command_native_export_report_bundle(root, report_id=report_id, output=output, status_only=True)
            content = output.read_bytes()
    ...
    def test_native_export_report_bundle_cap_preserves_existing_output(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                ...
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="s" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                result = yafvsctl.command_native_export_report_bundle(root, report_id=report_id, output=output, max_items=1, overwrite=True)
            self.assertEqual(result["status"], "fail")
    ...
    def test_native_start_tasks_from_csv_requires_write_control_before_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_start_tasks_from_csv(root, csv_file)
                curl.assert_not_called()
    ...
    def test_native_start_tasks_from_csv_paginates_skips_active_rows_and_aggregates_start_failures(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                ...
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="a" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                result = yafvsctl.command_native_start_tasks_from_csv(root, csv_file, allow_write_control=True)
    ...
    def test_native_bulk_stop_commands_require_write_control_before_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                csv_result = yafvsctl.command_native_stop_tasks_from_csv(root, csv_file)
                all_result = yafvsctl.command_native_stop_all_tasks(root)
                curl.assert_not_called()
    ...
    def test_native_stop_tasks_from_csv_snapshots_resolves_safely_and_aggregates_failures(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                ...
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="a" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                result = yafvsctl.command_native_stop_tasks_from_csv(
                    root,
                    csv_file,
                    allow_write_control=True,
                )
    ...
    def test_native_stop_all_tasks_filters_deduplicates_and_continues_after_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                ...
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="a" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                result = yafvsctl.command_native_stop_all_tasks(
                    root,
                    allow_write_control=True,
                    status_only=True,
                )
    ...
    def test_native_targets_from_host_list_parses_host_file_and_port_ranges(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                yafvsctl.load_native_target_host_list(hosts_file),
        ...
        for bad_range in ("1-443", "T:0-1", "U:10-9", ","):
            with self.subTest(bad_range=bad_range):
                with self.assertRaises(ValueError):
                    yafvsctl.parse_native_target_host_list_port_ranges(bad_range)
    ...
    def test_native_targets_from_host_list_dry_run_plans_secret_free_payloads(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            hosts_file.write_text("192.0.2.10\n192.0.2.11\n", encoding="utf-8")
            result = yafvsctl.command_native_targets_from_host_list(
                root,
                hosts_file,
    ...
    def test_native_targets_from_host_list_requires_write_control_before_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            hosts_file.write_text("192.0.2.10\n", encoding="utf-8")
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_targets_from_host_list(root, hosts_file)
                curl.assert_not_called()
    ...
    def test_native_credentials_from_csv_rejects_every_invalid_row_before_runtime(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for content in invalid_rows:
                ...
                with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                    result = yafvsctl.command_native_credentials_from_csv(root, csv_file, allow_write_control=True)
                    curl.assert_not_called()
                self.assertEqual(result["status"], "fail")
    ...
    def test_native_credentials_from_csv_defaults_to_redacted_no_runtime_dry_run(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            csv_file.chmod(0o600)
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_credentials_from_csv(root, csv_file)
                curl.assert_not_called()
    ...
    def test_native_credentials_from_csv_rejects_contradictory_write_and_dry_run_flags(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_credentials_from_csv(
                    root,
                    csv_file,
                    allow_write_control=True,
                    dry_run=True,
    ...
    def test_native_credentials_from_csv_rejects_unsafe_key_paths_and_boundaries(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "unsafe group or world permissions"):
                yafvsctl.load_native_credential_csv_rows(csv_file)

            key_file.chmod(0o600)
            key_file.write_bytes(b"x" * (yafvsctl.NATIVE_CREDENTIAL_CSV_MAX_PRIVATE_KEY_BYTES + 1))
    ...
    def test_native_credentials_from_csv_requires_private_regular_csv_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "must not be a symlink"):
                yafvsctl.load_native_credential_csv_rows(link_file)
    ...
    def test_native_credentials_from_csv_stops_on_first_post_failure_and_redacts_server_detail(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                result = yafvsctl.command_native_credentials_from_csv(root, csv_file, allow_write_control=True, status_only=True)
            ...
            with self.subTest(response_body=response_body):
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    with (
                        unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}),
                        unittest.mock.patch.object(yafvsctl, "native_api_direct_config_shape_finding", return_value=yafvsctl.finding("pass", "direct-config", "ok")),
                        unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="d" * 64),
                        unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct),
                    ):
                        result = yafvsctl.command_native_credentials_from_csv(root, csv_file, allow_write_control=True)
    ...
    def test_native_credential_csv_response_detects_escaped_and_unicode_secret_values(self):
        ...
        for submitted_secret in ('p"ass', "pässword", "line1\nline2", "back\\slash"):
            with self.subTest(submitted_secret=submitted_secret):
                row = yafvsctl.NativeCredentialCsvRow(1, "Created", "UP", "operator", submitted_secret, "")
                parsed = {
                    "id": credential_id,
                    "name": "Created",
                    "credential_type": "up",
                ...
                self.assertFalse(
                    yafvsctl.native_credential_csv_response_is_redacted(parsed, row, submitted_secret, "")
                )

        clean_row = yafvsctl.NativeCredentialCsvRow(1, "Created", "UP", "operator", "secret", "")
        clean_response = {"id": credential_id, "name": "Created", "credential_type": "up", "owner": "admin"}
        self.assertTrue(yafvsctl.native_credential_csv_response_is_redacted(clean_response, clean_row, "secret", ""))
            ...
            with self.subTest(returncode=post_response.returncode, output=post_response.stdout):
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    with (
                        unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}),
                        unittest.mock.patch.object(yafvsctl, "native_api_direct_config_shape_finding", return_value=yafvsctl.finding("pass", "direct-config", "ok")),
                        unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="f" * 64),
                        unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct),
                    ):
                        result = yafvsctl.command_native_credentials_from_csv(root, csv_file, allow_write_control=True)
    ...
    def test_native_credentials_from_csv_accounts_for_transport_exception_after_partial_write(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}),
                unittest.mock.patch.object(yafvsctl, "native_api_direct_config_shape_finding", return_value=yafvsctl.finding("pass", "direct-config", "ok")),
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="e" * 64),
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct),
            ):
                result = yafvsctl.command_native_credentials_from_csv(root, csv_file, allow_write_control=True)
    ...
    def test_native_credentials_from_csv_enforces_document_bounds(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            csv_file.chmod(0o600)
            with unittest.mock.patch.object(yafvsctl, "NATIVE_CREDENTIAL_CSV_MAX_ROWS", 1):
                with self.assertRaisesRegex(ValueError, "exceeds 1 rows"):
                    yafvsctl.load_native_credential_csv_rows(csv_file)
            with unittest.mock.patch.object(yafvsctl, "NATIVE_CREDENTIAL_CSV_MAX_AGGREGATE_SECRET_BYTES", 4):
    ...
    def test_native_schedules_from_csv_parses_rows_and_dry_runs_without_calendar_payloads(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_schedules_from_csv(root, csv_file, dry_run=True)
                curl.assert_not_called()

        self.assertEqual(result["status"], "pass")
        planned = result["details"]["planned_schedules"][0]
        self.assertEqual(planned["name"], "Nightly")
        self.assertEqual(planned["comment"], yafvsctl.NATIVE_SCHEDULE_CSV_COMMENT)
    ...
    def test_native_schedules_from_csv_rejects_malformed_rows_before_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_schedules_from_csv(root, csv_file, allow_write_control=True)
                curl.assert_not_called()
    ...
    def test_native_schedules_from_csv_matches_api_byte_and_control_boundaries(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows = yafvsctl.load_native_schedule_csv_rows(valid)
            ...
            self.assertEqual(yafvsctl.native_schedule_csv_schedule_body(rows[0])["icalendar"], calendar)
            ...
            with self.assertRaisesRegex(ValueError, "exceeds 4096"):
                yafvsctl.load_native_schedule_csv_rows(too_long)
            ...
            with self.assertRaisesRegex(ValueError, "unsupported control"):
                yafvsctl.load_native_schedule_csv_rows(bad_calendar)
            ...
            with self.assertRaisesRegex(ValueError, "schedule name must be printable"):
                yafvsctl.load_native_schedule_csv_rows(bad_name)

            blank_timezone = root / "blank-timezone.csv"
            blank_timezone.write_text("Default timezone,,BEGIN:VCALENDAR\n", encoding="utf-8")
            self.assertEqual(yafvsctl.load_native_schedule_csv_rows(blank_timezone)[0].timezone, "")
    ...
    def test_native_schedules_from_csv_requires_explicit_write_confirmation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_schedules_from_csv(root, csv_file)
                curl.assert_not_called()
    ...
    def test_native_schedules_from_csv_posts_exact_native_request_shape(self):
        ...
        self.assertEqual(
            ...
            {
                "name": "Nightly",
                "comment": yafvsctl.NATIVE_SCHEDULE_CSV_COMMENT,
    ...
    def test_native_schedules_from_xml_parses_direct_rows_and_redacts_calendar_dry_run(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_schedules_from_xml(root, xml_file, dry_run=True)
                curl.assert_not_called()
    ...
    def test_native_schedules_from_xml_rejects_invalid_or_nested_rows_before_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_schedules_from_xml(root, xml_file, allow_write_control=True)
                curl.assert_not_called()
        self.assertEqual(result["status"], "fail")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_schedules_from_xml(root, xml_file)
                curl.assert_not_called()
        self.assertEqual(result["status"], "fail")
    ...
    def test_native_schedules_from_xml_preflights_and_posts_exact_shape(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}),
                unittest.mock.patch.object(yafvsctl, "native_api_direct_config_shape_finding", return_value=yafvsctl.finding("pass", "direct-config", "ok")),
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="x" * 64),
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct),
            ):
                result = yafvsctl.command_native_schedules_from_xml(root, xml_file, allow_write_control=True)
...
""",
    ...
    def test_native_tags_from_csv_parses_supported_subset_and_dry_runs_payloads(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows = yafvsctl.load_native_tag_csv_rows(csv_file)
            self.assertEqual(len(rows), 1)
            ...
            result = yafvsctl.command_native_tags_from_csv(root, csv_file, dry_run=True)
    ...
    def test_native_tags_from_csv_rejects_report_rows_without_exact_resources(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl") as curl:
                result = yafvsctl.command_native_tags_from_csv(root, csv_file)
                curl.assert_not_called()
    ...
    def test_native_tags_from_csv_accepts_report_rows_with_exact_resources(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(yafvsctl, "native_api_direct_runtime_env", return_value={}), \
                ...
                unittest.mock.patch.object(yafvsctl, "native_api_direct_bearer_token", return_value="a" * 64), \
                unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                result = yafvsctl.command_native_tags_from_csv(root, csv_file, allow_write_control=True, status_only=True)
    ...
    def test_native_tags_from_csv_exact_lookup_pages_until_match(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with unittest.mock.patch.object(yafvsctl, "direct_native_api_curl", side_effect=fake_direct):
                matches, failure = yafvsctl.native_tag_csv_fetch_exact_items(
                    root,
    ...
    def test_retained_json_artifacts_write_latest_history_and_prune(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            latest, first = yafvsctl.retained_json_artifact_paths(artifact_dir, "quality-gate", "quality-gate.json")
            yafvsctl.write_retained_json_artifact(latest, first, {"status": "pass", "summary": "first", "metadata": {"generated_at": "one"}}, "quality-gate", 1)
            latest, second = yafvsctl.retained_json_artifact_paths(artifact_dir, "quality-gate", "quality-gate.json")
            yafvsctl.write_retained_json_artifact(latest, second, {"status": "fail", "summary": "second", "metadata": {"generated_at": "two"}}, "quality-gate", 1)

            self.assertTrue((artifact_dir / "quality-gate.json").is_file())
            history = yafvsctl.json_artifact_history(artifact_dir, "quality-gate")
    ...
    def test_runtime_secret_helper_accepts_default_value(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "TurboVAS"
            root.mkdir()
            secret, created = yafvsctl.read_or_create_runtime_secret(root, "example", "admin")
            ...
            metadata = yafvsctl.runtime_secret_file_metadata(secret_path)
    ...
    def test_env_values_have_nonempty_key_rejects_empty_compose_values(self):
        ...
        self.assertFalse(yafvsctl.env_values_have_nonempty_key(["TOKEN"], "TOKEN"))
        self.assertFalse(yafvsctl.env_values_have_nonempty_key(["TOKEN="], "TOKEN"))
        self.assertFalse(yafvsctl.env_values_have_nonempty_key(["TOKEN=   "], "TOKEN"))
    ...
    def test_direct_native_api_posture_keeps_default_internal_mode_passing(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "TurboVAS"
            root.mkdir()
            with unittest.mock.patch.object(yafvsctl, "current_native_api_direct_published_bindings", return_value=()), unittest.mock.patch.object(yafvsctl, "running_service_env_has_key", return_value=False):
                findings = yafvsctl.direct_native_api_posture_findings(root, {})
    ...
    def test_validate_direct_api_request_id_rejects_unsafe_values(self):
        self.assertEqual(
            yafvsctl.validate_direct_api_request_id("client-123_abc.4:5"),
            "client-123_abc.4:5",
        )
        for value in ("", "contains space", "../bad", "a" * 129):
            with self.subTest(value=value):
                with self.assertRaises(ValueError):
                    yafvsctl.validate_direct_api_request_id(value)
...
"""
    ...
    def test_gsa_static_staging_writes_browser_relative_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                yafvsctl.os.environ[yafvsctl.GSAD_HOSTS_ENV] = "192.168.178.42,100.80.139.13"
                findings = yafvsctl.stage_gsa_static(root)
            finally:
                if original is None:
                    yafvsctl.os.environ.pop(yafvsctl.GSAD_HOSTS_ENV, None)
                else:
                    yafvsctl.os.environ[yafvsctl.GSAD_HOSTS_ENV] = original
            self.assertEqual(yafvsctl.aggregate_status(findings), "pass")
            config = yafvsctl.gsad_static_dir(root) / "config.js"
    ...
    def test_feed_community_key_download_command_targets_runtime_artifact(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "TurboVAS"
            root.mkdir()
            command = yafvsctl.feed_community_key_download_command(root)
            self.assertEqual(command[:3], ["curl", "-fsSL", "-o"])
            self.assertEqual(command[3], str(yafvsctl.feed_community_key_path(root)))
            self.assertEqual(command[4], yafvsctl.GREENBONE_COMMUNITY_KEY_URL)

    def test_notus_signature_files_use_active_generation(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "TurboVAS"
            root.mkdir()
            signature_files = yafvsctl.notus_signature_files(root)
            self.assertEqual(
                signature_files,
                [
                    (
    ...
    def test_feed_activation_state_is_private_atomic_and_validated(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path = yafvsctl.feed_activation_state_path(root)
            ...
            with unittest.mock.patch.object(
                yafvsctl.os, "fsync", side_effect=record_fsync
            ):
                yafvsctl.write_feed_activation_state(root, payload)

            self.assertIn(str(path.parent.parent), fsynced_paths)
            self.assertEqual(stat.S_IMODE(path.parent.stat().st_mode), 0o700)
            self.assertEqual(stat.S_IMODE(path.stat().st_mode), 0o600)
            self.assertEqual(
                yafvsctl.read_feed_activation_state(root)["current_generation_id"],
                "a" * 64,
            )
            with self.assertRaisesRegex(ValueError, "identifiers are invalid"):
                yafvsctl.write_feed_activation_state(
                    root,
    ...
    def test_missing_feed_activation_state_read_does_not_create_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIsNone(yafvsctl.read_feed_activation_state(root))
    ...
    def test_feed_state_reports_missing_cache_and_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "TurboVAS"
            root.mkdir()
            result = yafvsctl.command_feed_state(root)
            self.assertEqual(result["status"], "warn")
    ...
    def test_feed_image_snapshot_rejects_partial_running_deployment(self):
        running = {
            yafvsctl.APP_SERVICES[0]: "sha256:" + "a" * 64,
        }
        with unittest.mock.patch.object(
            yafvsctl,
            "container_image_id",
            ...
            image_ids, error = yafvsctl.resolve_app_service_image_ids(
                Path("/tmp"),
                prefer_running=True,
                app_env={},
            )
    ...
    def test_runtime_app_build_refuses_running_bind_mount_consumers(self):
        with unittest.mock.patch.object(
            yafvsctl, "running_app_services", return_value=("gvmd", "gsad")
        ), unittest.mock.patch.object(yafvsctl, "run_command") as run_command:
            result = yafvsctl._command_runtime_app_build_unlocked(Path("/tmp"))
    ...
    def test_feed_activation_state_validates_restore_gsad_hosts(self):
        ...
        self.assertEqual(
            yafvsctl.validate_feed_activation_state(dict(payload))[
                "restore_gsad_hosts"
            ],
    ...
    def test_full_test_scan_target_is_explicit_canonical_and_bounded(self):
        ...
        self.assertEqual(runtime_full_test_scan.IANA_TCP_UDP_PORT_LIST_ID, yafvsctl.IANA_TCP_UDP_PORT_LIST_ID)
