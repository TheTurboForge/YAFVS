...
const nativeLoadEntities = gmp => filter => (dispatch, getState) => {
  ...
  return fetchNativeFilters(gmp, nativeFiltersQueryFromFilter(filter)).then(
    response =>
      dispatch(
...
const nativeLoadEntity = gmp => id => (dispatch, getState) => {
  ...
  return fetchNativeFilter(gmp, id).then(
    filter => dispatch(entityLoadingActions.success(id, filter)),
