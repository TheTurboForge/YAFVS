...
| --- | --- | --- | --- |
| Raw report list/detail/results/hosts/ports/CVEs/errors | GSA GMP commands -> gsad -> gvmd XML -> PostgreSQL | `/api/v1/reports`, `/api/v1/reports/{report_id}`, `/api/v1/reports/{report_id}/results`, `/hosts`, `/ports`, `/cves`, and `/errors` | `/reports` now reads its list through typed JSON. Raw detail summary plus raw report Results, Hosts, Ports, CVEs, and Error Messages tab reads use native JSON through the authenticated `gsad` proxy; Applications, Operating Systems, TLS Certificates, and Closed CVEs remain inherited until migrated or retired. |
