...
| `just runtime-native-api-direct-write-smoke --status-only --json` | Guarded direct write-control can enable, run zero-residue approved write probes, and restore write-control state, including credential name/comment metadata patch coverage with sentinel secret-row preservation. Fixture-scarcity warnings for report-config/schedule positive probes are expected when the runtime lacks those records. | Production write authorization, scanner/feed/credential-secret/account control, destructive writes, or every possible retained write path. |
