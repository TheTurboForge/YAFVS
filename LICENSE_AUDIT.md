...
The native credential generator uses the non-vendored, exact-pinned
`ssh-key` crate for encrypted OpenSSH Ed25519 key encoding and the `zeroize`
crate for short-lived seed cleanup. Both dependencies are dual-licensed under
`Apache-2.0 OR MIT`; their exact versions and transitive dependency set are
recorded in `services/yafvs-api/Cargo.lock`. Dependency-security and license
gates remain required when either pin changes.
