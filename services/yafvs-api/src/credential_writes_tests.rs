...
fn create_request(credential_type: CredentialCreateType) -> CredentialCreateRequest {
    CredentialCreateRequest {
        ...
        password: (credential_type == CredentialCreateType::Up)
            ...
            .into(),
        passphrase: None.into(),
        private_key: (credential_type == CredentialCreateType::Usk)
            .then(|| serde_json::from_value(serde_json::json!("test key material\n")).unwrap())
            .into(),
        autogenerate: false,
    }
}

fn autogenerated_request(credential_type: CredentialCreateType) -> CredentialCreateRequest {
    CredentialCreateRequest {
        name: "generated credential".to_string(),
        comment: None,
        login: "operator".to_string(),
        credential_type,
        password: None.into(),
        passphrase: None.into(),
        private_key: None.into(),
        autogenerate: true,
    }
}
...
fn credential_create_validates_up_and_usk_without_echoing_secrets() {
    ...
    usk_request.passphrase = Some(serde_json::from_value(serde_json::json!("")).unwrap()).into();
    ...
    assert_eq!(usk.private_key.as_bytes(), b"test key material\n");
...
fn credential_create_command_uses_drop_scrubbing_across_the_await_boundary() {
    ...
    let create = source
        .split_once("pub(crate) async fn create_credential")
        .expect("credential create handler")
        .1
        .split_once("pub(crate) async fn hard_delete_credential")
        .expect("credential create handler boundary")
        .0;
    assert!(
        create
            .find("generate_credential_secrets(&mut request).await?")
            .unwrap()
            < create.find("request_credential_create(").unwrap(),
        "generation must complete before credential-create control dispatch"
    );
}
...
fn credential_create_rejects_cross_type_fields_unknown_fields_and_bad_bounds() {
    ...
    up.private_key = Some(serde_json::from_value(serde_json::json!("key")).unwrap()).into();
    assert!(matches!(
    ...
    usk.password = Some(serde_json::from_value(serde_json::json!("wrong field")).unwrap()).into();
    assert!(matches!(
    ...
    assert!(
        ...
        .is_err(),
        "autogeneration is strict to the UP/USK request type enum"
    );

    for credential_type in ["up", "usk"] {
        for mixed_field in ["password", "passphrase", "private_key"] {
            for mixed_value in [serde_json::json!(""), serde_json::Value::Null] {
                ...
                request[mixed_field] = mixed_value;
                let request = serde_json::from_value::<CredentialCreateRequest>(request)
                    .expect("presence-aware request shape remains parseable");
                assert!(matches!(
...
#[tokio::test]
async fn credential_autogeneration_uses_os_random_ascii_secrets_and_scrubbed_frames() {
    let mut up =
        validate_credential_create_request(autogenerated_request(CredentialCreateType::Up))
            .expect("valid autogenerated up request");
    crate::credential_generation::generate_credential_secrets(&mut up)
        .await
        .expect("up secret generation");
    assert_eq!(up.secret.as_bytes().len(), 32);
    assert!(up.secret.as_bytes().iter().all(u8::is_ascii_alphanumeric));
    assert!(!up.secret.as_bytes().is_empty());
    assert!(up.private_key.as_bytes().is_empty());
    ...
    assert!(!command.as_bytes().is_empty());
    assert!(
        !command
            .as_bytes()
            .windows(up.secret.as_bytes().len())
            .any(|window| window == up.secret.as_bytes()),
        "the generated secret must be base64-framed, never present as plaintext"
    );
}

#[tokio::test]
async fn credential_autogeneration_creates_encrypted_openssh_ed25519_keys() {
    let mut usk =
        validate_credential_create_request(autogenerated_request(CredentialCreateType::Usk))
            .expect("valid autogenerated usk request");
    crate::credential_generation::generate_credential_secrets(&mut usk)
        .await
        .expect("usk key generation");

    assert_eq!(usk.secret.as_bytes().len(), 32);
    assert!(usk.secret.as_bytes().iter().all(u8::is_ascii_alphanumeric));
    let key_text = std::str::from_utf8(usk.private_key.as_bytes()).expect("OpenSSH text");
    assert!(key_text.starts_with("-----BEGIN OPENSSH PRIVATE KEY-----"));
    let encrypted = ssh_key::PrivateKey::from_openssh(key_text).expect("parse encrypted key");
    assert!(encrypted.is_encrypted());
    let decrypted = encrypted
        .decrypt(usk.secret.as_bytes())
        .expect("generated passphrase decrypts generated key");
    assert_eq!(decrypted.algorithm(), ssh_key::Algorithm::Ed25519);
