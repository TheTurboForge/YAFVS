...
fn python_version_finding(runner: &dyn CommandRunner, repo_root: &Path) -> Finding {
    ...
    Finding::new(
        ...
        "tool.python-version",
        if passed {
            format!("python3 version {version} satisfies TurboVAS tooling requirement >= 3.11.")
        } else {
            format!("python3 version {version} is older than TurboVAS tooling requirement >= 3.11; TOML-backed checks need Python 3.11+.")
        },
    )
