...
const API_CONTRACT: &str = include_str!("../../../docs/API_CONTRACT.md");
const GMP_XML_STRANGLER: &str = include_str!("../../../docs/GMP_XML_STRANGLER.md");
...
fn credential_autogeneration_is_browser_native_and_raw_generation_is_retired() {
    ...
    for required in [
        "autogenerate:",
        "default: false",
        "password, passphrase, and private_key must all be omitted",
    ] {
        assert!(
            schema.contains(required),
            "OpenAPI marker missing: {required}"
        );
    }
    for documentation in [API_CONTRACT, GMP_XML_STRANGLER] {
        assert!(
            documentation
                .contains("browser uses that route unconditionally for manual and autogenerated")
...
#[test]
fn credential_create_gsa_routes_manual_and_autogenerated_up_and_usk_to_native_api() {
    for required in [
    ...
    assert!(
        !create.contains("canUseNativeApi(this.http) &&"),
        "UP/USK type selection must not be gated by native capability"
    );
    assert!(
        create.contains("if (!canUseNativeApi(this.http))"),
        "UP/USK creation must fail closed when native capability is unavailable"
    );
    assert!(
        create
            .contains("const baseData = this.createBase(args);\n    return this.action(baseData);"),
        "other credential types must retain inherited create_credential"
    );
    assert!(
    ...
    let gsad_create = GSAD_GMP_C
        ...
        .0;
    let native_only_rejection = gsad_create
        .find("Username/password and SSH-key credential creation must use the")
        .expect("UP/USK browser-bridge rejection");
    let retained_manager_send = gsad_create
        .find("ret = gmp")
        .expect("retained other-type manager send");
    assert!(
        native_only_rejection < retained_manager_send,
        "UP/USK browser creation must be rejected before any retained manager send"
    );
    assert!(!gsad_create.contains("if (str_equal (type, \"up\"))"));
    assert!(!gsad_create.contains("else if (str_equal (type, \"usk\"))"));
    for retained_type in ["krb5", "cc", "snmp", "pgp", "smime"] {
        assert!(
            gsad_create.contains(&format!("str_equal (type, \"{retained_type}\")")),
            "retained gsad credential-create type is missing: {retained_type}"
        );
    }

    assert!(GVMD_GMP_C.contains("case CLIENT_CREATE_CREDENTIAL:"));
    assert!(GMP_SCHEMA.contains("<name>create_credential</name>"));
    for retained_client in [
        "gmp_create_lsc_credential (",
        "gmp_create_lsc_credential_key (",
        "gmp_create_lsc_credential_ext (",
    ] {
        assert!(
            GVM_LIBS_GMP_C.contains(retained_client),
            "raw manual credential client compatibility is missing: {retained_client}"
        );
    }
}
