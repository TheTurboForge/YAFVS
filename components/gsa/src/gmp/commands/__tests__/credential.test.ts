...
describe('CredentialCommand tests', () => {
  ...
  test('should fail closed for manual UP and USK creation when the native API is unavailable', async () => {
    ...
    for (const credentialType of [
      ...
      ).rejects.toThrow('Native credential creation is unavailable for UP/USK');
    }
  ...
  test('should fail closed for autogenerated UP and USK creation when the native API is unavailable', async () => {
    const fetchMock = testing.fn();
    testing.stubGlobal('fetch', fetchMock);
    const fakeHttp = createHttp(undefined);
    const cmd = new CredentialCommand(fakeHttp);
