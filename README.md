...
| Rust operator CLI | Incremental migration | Rust implements 103 parity-tested subcommands across repository, assurance, build orchestration, runtime inspection, guarded resource/task/scan control, immutable feed staging, and explicit opt-in operator actions; Python remains the compatibility entrypoint for the rest. |
