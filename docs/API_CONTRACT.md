...
native POST without a GMP fallback for manual and autogenerated
username/password and username/SSH-key creation, and native PATCH for effective
metadata-only saves, including realistic form state. Native autogeneration uses
the OS CSPRNG for 32-character ASCII alphanumeric UP passwords and USK
passphrases, and an encrypted OpenSSH Ed25519 private key; those values remain
write-only and are never returned. Any secret update, login change, key or
