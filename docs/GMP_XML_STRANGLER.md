...
| --- | --- | --- | --- |
| Raw report list/detail/results/hosts/ports | GSA GMP commands -> gsad -> gvmd XML -> PostgreSQL | `/api/v1/reports`, `/api/v1/reports/{report_id}`, `/api/v1/reports/{report_id}/results`, `/api/v1/reports/{report_id}/hosts`, and `/api/v1/reports/{report_id}/ports` | `/reports` now reads its list through typed JSON. Raw detail summary plus raw report Results, Hosts, and Ports tab reads use native JSON through the authenticated `gsad` proxy; remaining heavy detail tabs stay inherited until migrated in follow-ups. |
