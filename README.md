...
| Rust operator CLI | Experimental proof | Rust owns parity-tested read-only `status` and `inventory`; Python remains the compatibility entrypoint for orchestration. |
...
The optional Rust CLI migration proof should match the Python `status`
contract:
