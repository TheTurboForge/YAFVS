...
pub(crate) async fn delete_tls_certificate(
    ...
    let operator_owner_id = resolve_tls_certificate_write_operator_owner(&tx, &operator).await?;
    tx.batch_execute(
    ...
    ensure_tls_certificate_owner_matches_operator(certificate_state.owner_id, operator_owner_id)?;
