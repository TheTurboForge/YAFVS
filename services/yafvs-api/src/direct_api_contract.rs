...
pub(crate) fn direct_api_v1_path_is_allowed(path: &str) -> bool {
    ...
    matches!(
        ...
        ["", "api", "v1", "results"]
            ...
            | ["", "api", "v1", "vulnerabilities", _]
            | ["", "api", "v1", "vulnerabilities", _, "export"]
            | ["", "api", "v1", "cves", _]
            ...
            | ["", "api", "v1", "tls-certificates", _, "certificate"]
            | ["", "api", "v1", "scanners", _]
            ...
            | ["", "api", "v1", "alerts", _, "restore"]
            | ["", "api", "v1", "alerts", _, "trash"]
            | ["", "api", "v1", "tags", _]
            | ["", "api", "v1", "overrides", _]
            ...
            | ["", "api", "v1", "scope-reports", _, "results"]
...
fn direct_api_credential_public_key_path_is_allowed(path: &str) -> bool {
    let parts = path.split('/').collect::<Vec<_>>();
    matches!(
        parts.as_slice(),
...
fn direct_api_canonical_uuid_segment_is_allowed(segment: &str) -> bool {
    Uuid::parse_str(segment).is_ok_and(|uuid| uuid.to_string() == segment)
}
...
fn direct_api_v1_write_method_path_is_allowed(method: &Method, path: &str) -> bool {
    ...
    match (method, parts.as_slice()) {
        ...
        (&Method::POST, ["", "api", "v1", "alerts", alert_id, "restore"]) => {
            direct_api_write_id_segment_is_allowed(alert_id)
        }
        (&Method::DELETE, ["", "api", "v1", "alerts", alert_id, "trash"]) => {
            direct_api_write_id_segment_is_allowed(alert_id)
        }
        ...
        (&Method::POST, ["", "api", "v1", "scan-configs", "import"]) => true,
        ...
        (&Method::PATCH | &Method::DELETE, ["", "api", "v1", "credentials", credential_id]) => {
        ...
        (&Method::POST, ["", "api", "v1", "scanners", scanner_id, "clone"])
        | (&Method::POST, ["", "api", "v1", "scanners", scanner_id, "restore"])
        | (&Method::DELETE, ["", "api", "v1", "scanners", scanner_id, "trash"]) => {
            direct_api_write_id_segment_is_allowed(scanner_id)
        }
        (
            &Method::POST,
        ...
        (&Method::POST, ["", "api", "v1", "tasks"]) => true,
        (&Method::POST, ["", "api", "v1", "tasks", task_id, "clone"]) => {
        ...
        (&Method::POST, ["", "api", "v1", "tasks", task_id, "stop"]) => {
            direct_api_write_id_segment_is_allowed(task_id)
        }
        ...
        (&Method::PATCH | &Method::DELETE, ["", "api", "v1", "hosts", host_id]) => {
        ...
        (&Method::POST, ["", "api", "v1", "port-lists", port_list_id, "ranges"]) => {
            direct_api_write_id_segment_is_allowed(port_list_id)
        }
        (
            &Method::DELETE,
